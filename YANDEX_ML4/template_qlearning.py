...
def my_softmax(values: np.ndarray, T=1.):
    ...
    """
    nice_vector = (values - np.max(values))/T
    exp_vector = np.exp(nice_vector)
    exp_denominator = np.sum(exp_vector)
    probas = exp_vector / exp_denominator
    return probas
...
class QLearningAgent:
    ...
    def get_value(self, state):
        ...
        value = np.max([self._qvalues[state][i] for i in possible_actions])
    ...
    def get_best_action(self, state):
        ...
        best_action = max(possible_actions, key = lambda i: self._qvalues[state][i])
    ...
    def get_softmax_policy(self, state):
        ...
        q_values = [self._qvalues[state][i] for i in possible_actions]
        assert q_values is not None
        probabilities = my_softmax(np.array(q_values),T=self.temp)
    ...
    def get_action(self, state):
        ...
        chosen_action = np.random.choice(possible_actions, p=self.get_softmax_policy(state))
...
class EVSarsaAgent(QLearningAgent):
    ...
    def get_value(self, state):
        ...
        value = sum([self.get_softmax_policy(state)[i] * self.get_qvalue(state, possible_actions[i]) for i in range(len(possible_actions))])
        assert value is not None
